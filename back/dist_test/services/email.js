...
exports.sendRecordingEmail = exports.sendWelcomeEmail = exports.sendNotesEmail = exports.sendSpectatorRegistrationEmail = exports.sendRegistrationEmail = void 0;
...
function sendEmail(to, subject, html) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (process.env.SENDGRID_API_KEY) {
            ...
            yield mail_1.default.send({ from, to, subject, html });
        }
        else {
            ...
            yield transporter.sendMail({ from: `"Games of Senses" <${fromEmail}>`, to, subject, html });
        }
    });
}
const logoHtml = () => `
...
function buildGameHtml(playerName, game, siteUrl) {
    const statsRows = [];
...
function sendRegistrationEmail(to, playerName, game) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!EMAIL_ENABLED)
            return;
        const siteUrl = process.env.CLIENT_URL || 'http://localhost:3000';
        yield sendEmail(to, `🎮 Реєстрацію підтверджено — ${game.title} | Код: ${game.gameCode}`, buildGameHtml(playerName, game, siteUrl));
    });
}
exports.sendRegistrationEmail = sendRegistrationEmail;
function sendSpectatorRegistrationEmail(to, spectatorName, game) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!EMAIL_ENABLED)
            return;
        const siteUrl = process.env.CLIENT_URL || 'http://localhost:3000';
  ...
  <tr><td align="center" style="padding:40px 20px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:14px 32px;border-top:1px solid rgba(180,130,255,0.1);">
        ...
        yield sendEmail(to, `👁 Реєстрацію глядача підтверджено — ${game.title} | Код: ${game.spectatorCode}`, html);
    });
}
exports.sendSpectatorRegistrationEmail = sendSpectatorRegistrationEmail;
function sendNotesEmail(to, gmName, gameTitle, gameCode, notes) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!EMAIL_ENABLED)
            return;
        const notesHtml = notes
            .split('\n')
            .map(line => `<p style="margin:0 0 10px;font-size:13px;color:rgba(220,230,255,0.85);line-height:1.65;">${line.trim() ? line.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') : '&nbsp;'}</p>`)
            .join('');
        const html = `<!DOCTYPE html>
<html lang="uk">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
  ...
  <tr><td align="center" style="padding:40px 20px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:14px 32px;border-top:1px solid rgba(68,170,255,0.1);">
        ...
        yield sendEmail(to, `📝 Нотатки гри «${gameTitle}» (${gameCode}) — Games of Senses`, html);
    });
}
exports.sendNotesEmail = sendNotesEmail;
  ...
  <tr><td align="center" style="padding:40px 20px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:14px 32px;border-top:1px solid rgba(68,170,255,0.1);">
        ...
        yield sendEmail(to, '👾 Ласкаво просимо до Games of Senses!', html);
    });
}
exports.sendWelcomeEmail = sendWelcomeEmail;
function sendRecordingEmail(to, gameTitle, gameCode, shareLink) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!EMAIL_ENABLED)
            return;
  ...
  <tr><td align="center" style="padding:40px 20px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:14px 32px;border-top:1px solid rgba(68,170,255,0.1);">
        ...
        yield sendEmail(to, `🎬 Запис гри «${gameTitle}» (${gameCode}) — Games of Senses`, html);
    });
}
exports.sendRecordingEmail = sendRecordingEmail;

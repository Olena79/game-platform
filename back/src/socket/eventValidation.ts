...
export function validateSocketEvent<T>(schema: ZodSchema, handler: EventHandler<T>, socket?: Socket) {
	return async (data: any) => {
		...
		try {
			...
			// Zod 4 exposes `issues`; anything else is a failure inside the
			// handler, whose message is for the log, not for the client.
			const issues = error?.issues
			if (issues) {
				logger.warn('[socket validation]', { issues })
				...
				logger.error('[socket handler]', { error: error?.message ?? String(error) })
				socket?.emit('gr:action-error', 'Action failed')
			}
		}
	}
}
